...
def test_local_weights_success(mock_cuda, mock_isfile, mock_llama, mock_file_exists):
    ...
    result = local_models.get_local_weights(
        mock_file_exists, temperature, logger, verbose=False
    )
...
def test_download_model_from_huggingface_success(mock_download, mock_llama):
    ...
    result = local_models.download_model_from_huggingface(
        filename, repo_id, temperature, logger, verbose=False
    )
...
def test_download_model_from_huggingface_download_error(mock_download):
    ...
    with pytest.raises(ValueError) as exc_info:
        local_models.download_model_from_huggingface(
            filename, repo_id, temperature, logger, verbose=False
        )
...
def test_get_model_local_weights(mock_get_local, mock_llm_model):
    ...
    mock_get_local.assert_called_once_with(
        fake_path_to_local_weights, 0.7, logger, False
    )
...
def test_get_model_huggingface(mock_hf_download, mock_llm_model):
    ...
    mock_hf_download.assert_called_once_with(
        "llama-2-7b-chat.Q4.0.gguf", "TheBloke/Llama-2-7B-Chat-GGUF", 0.7, logger, False
    )
