...
class TestTextSearchEndpoint:
    ...
    def test_basic_text_search(
        self, mock_ts_rank_query, mock_get_session, mock_db_results
    ):
    ...
    @patch("routers.search_routes.get_session")
    @patch("routers.search_routes.ts_rank_query")
    def test_text_search_with_all_parameters(
        self, mock_ts_rank_query, mock_get_session, mock_db_results
    ):
    ...
    def test_text_search_empty_results(
        self, mock_ts_rank_query, mock_get_session, mock_empty_results
    ):
    ...
    @patch("routers.search_routes.get_session")
    @patch("routers.search_routes.ts_rank_query")
    def test_text_search_special_characters(
        self, mock_ts_rank_query, mock_get_session, mock_db_results
    ):
    ...
    @patch("routers.search_routes.get_session")
    @patch("routers.search_routes.ts_rank_query")
    def test_text_search_single_vocabulary_id(
        self, mock_ts_rank_query, mock_get_session, mock_db_results
    ):
    ...
    def test_text_search_ranking_order(self, mock_ts_rank_query, mock_get_session):
        ...
        results = [
            MockResult(
                "Concept A", 1, "codeA", "Domain", "Vocab", "Class", "S", None, 0.9
            ),
            MockResult(
                "Concept B", 2, "codeB", "Domain", "Vocab", "Class", "S", None, 0.8
            ),
            MockResult(
                "Concept C", 3, "codeC", "Domain", "Vocab", "Class", "S", None, 0.7
            ),
        ]
    ...
    @patch("routers.search_routes.get_session")
    @patch("routers.search_routes.ts_rank_query")
    def test_text_search_with_false_flags(
        self, mock_ts_rank_query, mock_get_session, mock_db_results
    ):
        ...
        response = client.get(
            "/text-search/diabetes",
            params={"standard_concept": False, "valid_concept": False},
        )
