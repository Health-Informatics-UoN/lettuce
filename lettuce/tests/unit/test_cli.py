...
def test_main_with_vector_search_and_llm(cli_runner, mock_llm_pipeline):
    ...
    with (
        patch("lettuce.cli.main.get_model") as mock_get_model,
        patch("time.time") as mock_time,
        patch(
            "huggingface_hub.snapshot_download",
            side_effect=lambda *args, **kwargs: AssertionError(
                "Hugging Face snapshot download detected"
            ),
        ),
        patch(
            "requests.get",
            side_effect=lambda *args, **kwargs: AssertionError("HTTP request detected"),
        ),
        ...
        assert mock_pipeline_instance.get_rag_assistant.called, (
            "get_rag_assistant was not called"
        )
...
def test_main_with_vector_search_only(cli_runner):
    ...
    with (
        ...
        mock_embeddings_instance.search.return_value = [
            [{"concept": "Aspirin info"}],
            [{"concept": "Tylenol info"}],
        ]
...
def test_cli_invalid_arguments(cli_runner):
    ...
    result = cli_runner.invoke(app, ["aspirin", "--search-threshold", "invalid_number"])
    assert result.exit_code != 0
...
def test_cli_output_format(cli_runner):
    ...
    with patch("lettuce.cli.main.OMOPMatcher") as mock_OMOPMatcher:
        ...
        result = cli_runner.invoke(
            app, ["aspirin", "--no-vector-search", "--no-use-llm"]
        )
