...
class OMOPMatcher:
    ...
    def fetch_omop_concepts(self, search_term: str) -> List[OMOPConcept] | None:
        ...
        for row_tuple in results:
            ...
            row.concept_name_similarity_score = self.calculate_similarity_score(
                row.concept_name, search_term
            )
            row.concept_synonym_name_similarity_score = self.calculate_similarity_score(
                row.concept_synonym_name, search_term
            )
        ...
        filtered_rows = [
            row for row in concept_rows if row.concept_id in concept_ids_above_threshold
        ]

        # Sort by highest similarity score
        filtered_rows.sort(
            key=lambda row: max(
                row.concept_name_similarity_score,
    ...
    def _format_concept_results(
        ...
        for _, rows in grouped.items():
            ...
            synonyms = [
                ...
                for row in rows
                if row.concept_synonym_name is not None
            ]
        ...
        if self.concept_ancestor:
            for concept in formatted_results:
                concept.concept_ancestor = self.fetch_concept_ancestors_and_descendants(
                    concept.concept_id
                )

        if self.concept_relationship:
            for concept in formatted_results:
                concept.concept_relationship = self.fetch_concept_relationships(
                    concept.concept_id
                )
