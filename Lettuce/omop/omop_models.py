...
def build_query(search_term: str, vocabulary_id: list[str] | None, concept_synonym: str) -> Select:
    ...
    if concept_synonym == 'y':
        ...
        synonym_matches = (
            select(ConceptSynonym.concept_id.label('synonym_concept_id'), ConceptSynonym.concept_synonym_name)
            .join(Concept, ConceptSynonym.concept_id == Concept.concept_id)
            .where(Concept.standard_concept == 'S')
            .where(synonym_ts_condition.bindparams(search_term=search_term))
        )
        if vocabulary_id:
            synonym_matches = synonym_matches.where(Concept.vocabulary_id.in_(vocabulary_id))
        ...
        query = query.outerjoin(synonym_matches_cte, Concept.concept_id == synonym_matches_cte.c.synonym_concept_id)
