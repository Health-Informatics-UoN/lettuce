...
class PostgresConceptExtractor(ConceptReader):
    ...
    def __init__(
        self, db_connector: PGConnector, batch_size: int, logger: Logger
    ) -> None:
        self.db_connector = db_connector
        self.logger = logger
        self._concept_query = sql.SQL(
            "SELECT concept_id, concept_name, domain_id, vocabulary_id, concept_class_id FROM {}"
        ).format(sql.Identifier(self.db_connector.db_schema, "concept"))
