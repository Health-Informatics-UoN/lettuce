...
def get_model(
    ...
    try:
        ...
        if "gpt" in model_name:
            ...
            # ------ Load the model from the local models dictionary ------ >
            try:
                model_path = hf_hub_download(**local_models[model_name])
                ...
                logger.info(f"Loaded {model_name} from local models.")
                return llm
            except Exception as e:
                logger.error(f"Error loading model {model_name}: {e}")
                raise RuntimeError(f"Failed to load the model {model_name}.")
