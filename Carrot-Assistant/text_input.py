...
# The session state is used to keep track of the state of the session.

# If the results of the lettuce are displayed or the user doesn't want to use LLM, the session is ended.
...
if st.button("Send"):
    if informal_names:
        ...
        with st.spinner("Processing..."):
            ...
            result_stream = make_api_call(names_list, use_llm=False, end_session=False, vocab_id=vocab_id)
            no_match_names = []
...
if "no_match_names" in st.session_state and not st.session_state.get(
    ...
    if user_choice == "No":
        with st.spinner("Ending session..."):
            ...
            # Make a final API call with to close the database connection.
            result_stream = make_api_call(
                st.session_state["no_match_names"], 
                use_llm=False,  
                end_session=True,  # Set end_session flag to True
                vocab_id=vocab_id
            )
            ...
            # Display final message before ending the session
            stream_message("<p style='color: #34A853;'>Thank you for using Carrot! 🥕</p>")
...
if st.session_state.get("llm_requested", False):
    ...
    with st.spinner("Processing with LLM..."):
        ...
        result_stream = make_api_call(no_match_names, use_llm=True, end_session=False, vocab_id=vocab_id)
    ...
    if llm_results:
        with st.spinner("Processing final OMOP query..."):
            new_result_stream = make_api_call(
                llm_results, use_llm=False, end_session=True, vocab_id=vocab_id
            )
