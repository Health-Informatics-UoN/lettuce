...
## `Generator`
This type is a wrapper for various Haystack generators.
This may or may not include `LlamaCppGenerator`, depending on your configuration.
...
|------------------|-----------------------------|-------------|
| `llm_model`       | `Generator`                  | An LLM generator |
...
| `inference_type` | `InferenceType` | The engine to use for inference, taken from [pipeline options](/api_reference/options/pipeline_options) |
| `inference_url` | `str` or `None` | A URL to use for inference, if you're using an external server. Defaults to the `ollama_url` from the environment |
